 ...
 *
 * Resolves syscall numbers at runtime by enumerating all Zw* exports from ntdll,
 * sorting them by address, and using the sorted index as the SSN. This works
 * because Windows assigns consecutive syscall numbers to Nt/Zw functions in the
 * order they appear in memory.
 *
...
bool InitSyscalls(void) {
    ...
    for (DWORD i = 0; i < pExports->NumberOfNames && g_SyscallCount < MAX_SYSCALL_ENTRIES; i++) {
    ...
    PSIZE_T RegionSize, ULONG AllocationType, ULONG Protect)
{
    ...
    return ((fn)RunSyscall)(ProcessHandle, BaseAddress, ZeroBits, RegionSize, AllocationType, Protect);
}
    ...
    SIZE_T NumberOfBytesToWrite, PSIZE_T NumberOfBytesWritten)
{
    ...
    return ((fn)RunSyscall)(ProcessHandle, BaseAddress, Buffer, NumberOfBytesToWrite, NumberOfBytesWritten);
}
    ...
    SIZE_T NumberOfBytesToRead, PSIZE_T NumberOfBytesRead)
{
    ...
    typedef NTSTATUS (NTAPI *fn)(HANDLE, PVOID, PVOID, SIZE_T, PSIZE_T);
    return ((fn)RunSyscall)(ProcessHandle, BaseAddress, Buffer, NumberOfBytesToRead, NumberOfBytesRead);
}
    ...
    ULONG NewProtection, PULONG OldProtection)
{
    ...
    return ((fn)RunSyscall)(ProcessHandle, BaseAddress, RegionSize, NewProtection, OldProtection);
}
    ...
    PVOID AttributeList)
{
    ...
    return ((fn)RunSyscall)(ThreadHandle, DesiredAccess, ObjectAttributes, ProcessHandle,
